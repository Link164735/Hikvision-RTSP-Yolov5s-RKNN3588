...
int main(int argc, char **argv)
{
  ...
  std::string rtsp_url = "rtsp://admin:xxx@xxx.xxx.xx.xx:xxx/Streaming/Channels/101?transport=tcp";
  ...
  int ith = 25;
  cap.set(cv::CAP_PROP_POS_FRAMES, ith);
  ...
  if (!cap.isOpened()) {
        ...
        printf( "Successfully opened video stream" );
  ...
  while (true) {
	...
	if (!ret){
	printf("Error: Could not read frame from video stream\n");
            // 尝试重新连接视频流
            if (retry_count < MAX_RETRIES && !ret) {
                system("bash /userdata/ok3588/yolo/yolov5s_infer_rknn_rk3588/rknn_yolov5_demo/net.sh");
                std::this_thread::sleep_for(std::chrono::seconds(2));
                cv::VideoCapture cap(rtsp_url);
  		int ith = 25;
  		cap.set(cv::CAP_PROP_POS_FRAMES, ith);
                printf("Retrying to reconnect...\n");
                cap.release(); // 释放之前的VideoCapture对象
                cap.open(rtsp_url); // 重新打开视频流
                bool ret = cap.read(frame);
                retry_count++;
